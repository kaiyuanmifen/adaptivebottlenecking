...
class ContrastiveSWM(nn.Module):
	...
	def __init__(self, embedding_dim, input_dims, hidden_dim, action_dim,
				 ...
				 Quantization=False,n_codebook_embedding=512,
				Quantization_method="VQVAE",n_quuantization_segments=1,
		...
		self.n_codebook_embedding=n_codebook_embedding
		self.Quantization_method=Quantization_method
		self.n_quuantization_segments=n_quuantization_segments
	...
	def energy(self, state, action, next_state, no_trans=False):
		...
		if no_trans:
			...
			if self.Quantization==True:
				pred_trans,CB_diff, CB_indexes= self.transition_model(state, action)
				diff = state + pred_trans - next_state  
				##return orignal energy plus codebook loss scaled to the same range and multipled by a adjusting factor
				#print("codebook loss",CB_diff)
				return (norm * diff.pow(2).sum(2).mean(1))+self.codebook_loss_weight*CB_diff
...
class TransitionGNN(torch.nn.Module):
	...
	def __init__(self, input_dim, hidden_dim, action_dim, num_objects,
				 ...
				 Quantization=False,n_codebook_embedding=512,Quantization_target="None",
				Quantization_method="VQVAE",n_quuantization_segments=1):
		...
		######informaiton for quantization
		self.Quantization=Quantization
		if self.Quantization:
			print("conduction Quantization:",str(Quantization))
			print("Quantization method",Quantization_method)
		else:
			print("not doing quantization")
		...
		if Quantization_target=="node_final":
			Target_dim=input_dim
		elif Quantization_target=="node_initial":
			Target_dim=input_dim
		elif Quantization_target=="node_action":
			Target_dim=input_dim+4 ##action dim =4 for 2D and 3D shapes
		else:
			Target_dim=self.hidden_dim
		if self.Quantization:
			if Quantization_method== "VQVAE":
				self.quantize=Quantize(Target_dim,n_codebook_embedding,groups=self.n_quuantization_segments)
			elif Quantization_method== "VQVAE_conditional":
				self.quantize=Quantize_conditional(Target_dim,n_codebook_embedding,groups=self.n_quuantization_segments)
			elif Quantization_method== "VQVAE_seperate":
				self.quantize=Quantize_separate(Target_dim,n_codebook_embedding,groups=self.n_quuantization_segments)
			elif Quantization_method== "VQVAE_conditional_seperate":
				self.quantize=Quantize_conditional_separate(Target_dim,n_codebook_embedding,groups=self.n_quuantization_segments)
			elif Quantization_method== "Gumbel":
				self.quantize=Quantize_gumbel(Target_dim,n_codebook_embedding,groups=self.n_quuantization_segments)
			elif Quantization_method== "Adaptive_Quantization":
				self.quantize=QuantizerFunction(input_dims=Target_dim,CodebookSize=n_codebook_embedding,Method=Quantization_method)
			elif Quantization_method== "Adaptive_Hierachical":
				self.quantize=QuantizerFunction(input_dims=Target_dim,CodebookSize=n_codebook_embedding,Method=Quantization_method)
			elif Quantization_method== "Quantization":
				self.quantize=QuantizerFunction(input_dims=Target_dim,CodebookSize=n_codebook_embedding,Method=Quantization_method)
			elif Quantization_method== "Original":
				self.quantize=QuantizerFunction(input_dims=Target_dim,CodebookSize=n_codebook_embedding,Method=Quantization_method)


			else:
				print("WRONG QUANTIZATION METHOD")

		if self.Quantization==True:
			print("conducting quantization")
			print("using method:"+self.Quantization_method)
			print("codebook size:",self.n_codebook_embedding)
			print("number of groups per embedding",self.n_quuantization_segments)
			print("target of quantization",self.Quantization_target)
	...
	def _node_model(self, node_attr, edge_index, edge_attr,Quantization=False,quantize=None):
		if edge_attr is not None:
			row, col = edge_index
			agg = utils.unsorted_segment_sum(
				edge_attr, row, num_segments=node_attr.size(0))
			if Quantization==True:
				###quantize the sume of edge attributes
				s1=agg.shape[0]
				s2=agg.shape[1]
				
				agg=agg.reshape(s1,1,s2)
				agg,CB_diff,CB_indexes=self.quantize(agg)
				agg=agg.reshape(s1,s2)
		...
		if Quantization==True:
			return self.node_mlp(out),CB_diff,CB_indexes
		else:
	...
	def forward(self, states, action):
		...
		if (self.Quantization==True) and (self.Quantization_target=="node_initial"):#quantizing the value of nodes before communication
				s1=node_attr.shape[0]
				s2=node_attr.shape[1]
				
				node_attr=node_attr.reshape(s1,1,s2)
				node_attr,CB_diff,CB_indexes=self.quantize(node_attr)
				node_attr=node_attr.reshape(s1,s2)
		...
		if num_nodes > 1:
			...
			if (self.Quantization==True) and (self.Quantization_target=="edge"):
				
				s1=edge_attr.shape[0]
				s2=edge_attr.shape[1]
				
				edge_attr=edge_attr.reshape(s1,1,s2)
				edge_attr,CB_diff,CB_indexes=self.quantize(edge_attr)
				edge_attr=edge_attr.reshape(s1,s2)
		...
		if (self.Quantization==True) and (self.Quantization_target=="node_action"):#quantizing nodes with action
				s1=node_attr.shape[0]
				s2=node_attr.shape[1]
		...
		if (self.Quantization==True) and (self.Quantization_target=="edge_sum"):
			node_attr,CB_diff,CB_indexes = self._node_model(
				node_attr, edge_index, edge_attr,self.Quantization,self.quantize)
		else:
			node_attr = self._node_model(
				node_attr, edge_index, edge_attr)


		if (self.Quantization==True) and (self.Quantization_target=="node_final"):#quantizing final output of node after communication
				s1=node_attr.shape[0]
				s2=node_attr.shape[1]
		...
		if self.Quantization==True:
			return node_attr.view(batch_size, num_nodes, -1),CB_diff, CB_indexes
