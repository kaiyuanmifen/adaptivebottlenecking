...
class ActorCritic_attentionSchema(nn.Module):
	def __init__(self,args, INPUT_DIM=3*3*3+2, HIDDEN_DIM=28,GWS_DIM=8, OUTPUT_DIM=4,key_dim=128):
		super().__init__()
...
for episode in range(1, MAX_EPISODES+1):
	...
	import csv   
	fields=[args.data,args.N_agents,args.hypothesis,episode,train_reward.mean().item(),test_reward.mean().item(),OODtest_reward.mean().item()]
	print("fileds")
	print(fields)
	...
	if episode % PRINT_EVERY == 0:
		...
		print(f'| Episode: {episode:3} | Mean Train Rewards: {train_reward.mean().item():5.1f} | Mean Test Rewards: {test_reward.mean().item():5.1f} | mean OOD Test Rewards: {OODtest_reward.mean().item():5.1f} |')
