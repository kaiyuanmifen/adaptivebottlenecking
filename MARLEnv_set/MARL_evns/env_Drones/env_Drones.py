...
class EnvDrones(object):
    ...
    def reset_drone_pos(self):
        for k in range(self.drone_num):
            self.drone_list[k].pos = [self.map_size-1, self.map_size-1]
    ...
    def drone_step(self, drone_act_list):
        ...
        for k in range(self.drone_num):
            if drone_act_list[k] == 0:
                if self.drone_list[k].pos[0] >= StepSize:
                    self.drone_list[k].pos[0] = self.drone_list[k].pos[0]-StepSize
            elif drone_act_list[k] == 1:
                if self.drone_list[k].pos[0] <= self.map_size - 1 -StepSize:
                    self.drone_list[k].pos[0] = self.drone_list[k].pos[0] + StepSize
            elif drone_act_list[k] == 2:
                if self.drone_list[k].pos[1] >= StepSize:
                    self.drone_list[k].pos[1] = self.drone_list[k].pos[1] - StepSize
            elif drone_act_list[k] == 3:
                if self.drone_list[k].pos[1] <= self.map_size -1-StepSize:
                    self.drone_list[k].pos[1] = self.drone_list[k].pos[1] + StepSize
    ...
    def human_step(self, human_act_list):
        ...
        for k in range(self.human_num):
            if human_act_list[k] == 0:
                if self.human_list[k].pos[0] >= StepSize:
                    free_space = self.land_mark_map[self.human_list[k].pos[0] - StepSize, self.human_list[k].pos[1]]
                    if free_space == 0:
                        self.human_list[k].pos[0] = self.human_list[k].pos[0] - StepSize
            elif human_act_list[k] == 1:
                if self.human_list[k].pos[0] <= self.map_size - StepSize-1:
                    free_space = self.land_mark_map[self.human_list[k].pos[0] + StepSize, self.human_list[k].pos[1]]
                    if free_space == 0:
                        self.human_list[k].pos[0] = self.human_list[k].pos[0] + StepSize
            elif human_act_list[k] == 2:
                if self.human_list[k].pos[1] >= StepSize:
                    free_space = self.land_mark_map[self.human_list[k].pos[0], self.human_list[k].pos[1] - StepSize]
                    if free_space == 0:
                        self.human_list[k].pos[1] = self.human_list[k].pos[1] - StepSize
            elif human_act_list[k] == 3:
                if self.human_list[k].pos[1] <= self.map_size - StepSize-1:
                    free_space = self.land_mark_map[self.human_list[k].pos[0], self.human_list[k].pos[1] + StepSize]
                    if free_space == 0:
                        self.human_list[k].pos[1] = self.human_list[k].pos[1] + StepSize
