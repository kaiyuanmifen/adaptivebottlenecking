...
from MARL_evns.env_MoveBox.env_MoveBox import EnvMoveBox
...
parser.add_argument('--data', type=str, default="MoveBox",
...
train_env = EnvMoveBox( agt1_pos=[13, 1],agt2_pos = [13, 13],box_pos = [10, 7])  
train_env.reset()


test_env = EnvMoveBox( agt1_pos=[13, 1],agt2_pos = [13, 13],box_pos = [10, 7])   # map_size, drone_num, view_range, tree_num, human_num
test_env.reset()



OODtest_env=EnvMoveBox( agt1_pos=[13, 3],agt2_pos = [13, 12],box_pos = [10, 5])   # map_size, drone_num, view_range, tree_num, human_num
OODtest_env.reset()
...
class ActorCritic_attentionSchema(nn.Module):
    def __init__(self,args, INPUT_DIM=29, HIDDEN_DIM=128,GWS_DIM=8, OUTPUT_DIM=4,key_dim=128):
        super().__init__()
...
def train(env, policy, optimizer, discount_factor, ppo_steps, ppo_clip):
    ...
    global_obs=(global_obs*255).astype(np.uint8)
    im = Image.fromarray(global_obs)
    im.save(args.data+"_global_obs_"+"Initial"+".png")
    ...
    for StepN in range(EpisodeLength):
        ...
        state=[]
        Obs_by_agents=env.get_obs()#observation of all agents 

        Positions_by_agents=torch.tensor(env.get_state().reshape(args.N_agents+1,2))
        
        
        for indx in range(args.N_agents):
            
            # im = Image.fromarray((Obs_by_agents[indx]*255).astype(np.uint8))
            # im.save(args.data+"_agent_obs_"+".png")
            Obs_agent=torch.FloatTensor(Obs_by_agents[indx]).unsqueeze(0)
            Obs_agent=Obs_agent.flatten().unsqueeze(0)
            
          
            allInfor=torch.cat([Obs_agent,Positions_by_agents[indx,:].unsqueeze(0)],1)
 
            state.append(allInfor)

        state=torch.cat(state,0).unsqueeze(1).float()#(N_Agent,1,state_dim)
        ...
        agent_act_list=action.flatten().tolist()
        
        # print("agent_act_list")
        # print(agent_act_list)
        reward, done = env.step(agent_act_list)
        ...
        #     global_obs=(global_obs*255).astype(np.uint8)
        #     im = Image.fromarray(global_obs)
        #     im.save(args.data+"_global_obs_"+"step"+str(StepN)+".png")
        ...
        if done:
        #when get +ve reward, game finishes
            break
    ...
    rewards=torch.tensor(rewards).unsqueeze(0).repeat(args.N_agents,1).unsqueeze(2).unsqueeze(3)
    rewards=rewards+(torch.randn(rewards.shape)/100000)
...
for episode in range(1, MAX_EPISODES+1):
    ...
    mean_train_rewards = train_rewards[0]
    mean_test_rewards = test_rewards[0]
    OODmean_test_rewards = OODtest_rewards[0]
