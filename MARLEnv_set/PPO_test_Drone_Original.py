...
class ActorCritic_attention(nn.Module):
	def __init__(self,args, INPUT_DIM=1183, HIDDEN_DIM=128, OUTPUT_DIM=4,key_dim=128):
		...
		self.actors=nn.ModuleList([MLP(INPUT_DIM, HIDDEN_DIM, OUTPUT_DIM) for i in range(args.N_agents)])
		self.critics=nn.ModuleList([MLP(INPUT_DIM, HIDDEN_DIM, 1) for i in range(args.N_agents)])
		
				

		self.query_projector=nn.Linear(INPUT_DIM, key_dim)
	...
	def forward(self, state):
		...
		return actions_pred,value_pred
...
for episode in range(1, MAX_EPISODES+1):
	...
	import csv   
	fields=[args.data,args.N_agents,episode,mean_train_rewards,mean_test_rewards,OODmean_test_rewards]
	print("fileds")
	print(fields)
