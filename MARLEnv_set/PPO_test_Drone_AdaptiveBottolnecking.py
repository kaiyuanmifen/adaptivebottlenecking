...
train_env = EnvDrones(map_size, N_agents, view_range, 10, 100)   # map_size, drone_num, view_range, tree_num, human_num
train_env.rand_reset_drone_pos()


test_env = EnvDrones(map_size, N_agents, view_range, 10, 100)   # map_size, drone_num, view_range, tree_num, human_num
...
class ActorCritic_attention(nn.Module):
	def __init__(self,args, INPUT_DIM=1183, HIDDEN_DIM=128, OUTPUT_DIM=4,key_dim=128):
		super().__init__()
	...
	def forward(self, state):
		...
		return actions_pred,value_pred,CBloss,att_scores.sum(1).squeeze(0).detach().clone()
...
def train(env, policy, optimizer, discount_factor, ppo_steps, ppo_clip):
	...
	for StepN in range(EpisodeLength):
		...
		action_pred,value_pred,CBloss, attscores = policy(state)
		All_CBLoss+=float(CBloss)
	...
	return policy_loss, value_loss, episode_reward,All_CBLoss,All_attScores
...
def update_policy(policy, states, actions, log_prob_actions, advantages, returns, optimizer, ppo_steps, ppo_clip):
	...
	for _ in range(ppo_steps):
		...
		action_pred, value_pred ,CBloss,attscores= policy(states)
...
for episode in range(1, MAX_EPISODES+1):
	...
	policy_loss, value_loss, train_reward ,CodebookLoss_train,AllAttScores_train= train(train_env, policy, optimizer, DISCOUNT_FACTOR, PPO_STEPS, PPO_CLIP)
	...
	import csv   
	fields=[args.data,args.N_agents,args.Method,episode,mean_train_rewards,mean_test_rewards,OODmean_test_rewards,CodebookLoss_train,AllAttScores_train.sum(0).tolist(),AllAttScores_test.sum(0).tolist(),AllAttScores_OODtest.sum(0).tolist()]
	print("fileds")
	print(fields)
	...
	if episode % PRINT_EVERY == 0:
		...
		print(f'| Episode: {episode:3} | Mean Train Rewards: {mean_train_rewards:5.1f} | Mean Test Rewards: {mean_test_rewards:5.1f} | Mean OOD Test Rewards: {OODmean_test_rewards:5.1f} |Codebook loss: {CodebookLoss_train:5.1f} ')
